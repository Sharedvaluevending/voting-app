...
```powershell
curl -X POST "YOUR_NGROK_URL/v1/chat/completions" -H "Content-Type: application/json" -H "ngrok-skip-browser-warning: 1" -H "User-Agent: VotingApp-Ollama/1.0" -d "{\"model\":\"llama3.2\",\"messages\":[{\"role\":\"user\",\"content\":\"Say hi\"}]}"
```

## 3. Test POST /api/generate
```powershell
curl -X POST "YOUR_NGROK_URL/api/generate" -H "Content-Type: application/json" -H "ngrok-skip-browser-warning: 1" -d "{\"model\":\"llama3.2\",\"prompt\":\"Say hi\"}"
```
